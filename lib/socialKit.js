...
Musubi = {
    _contexts: [],
    _encoding: "js", // argument encoding scheme. android requires 'string'
    _enc: function(msg) {
      if (Musubi._encoding == "js") return msg;
      if (Musubi._encoding == "string") { return JSON.stringify(msg); }
    },
    _launchCallback: null,
    _launch: function(argEncoding, user, feed, appId, message) {
        if (DBG) console.log("launching socialkit.js app");
        // The encoding used to shuttle objects to backend. Can be "js" or "string"
        Musubi._encoding = argEncoding;
    	Musubi.platform._setConfig(Musubi._enc({title: document.title}));
...
if (typeof Musubi_android_platform == "object") {
  Musubi.platform = Musubi_android_platform;
