...
Musubi.platform = {
	...
	_postObjToFeed: function(obj, session) {},
        _postObjToSubfeed: function(obj, session, parentId) {},
...
if (typeof Musubi_android_platform == "object") {
  ...
  Musubi.platform = {
    ...
    _querySubfeed: function(objId, query, sortOrder) {
	  return new SocialKit.DbObj(JSON.parse(Musubi_android_platform._querySubfeed(objId, query, sortOrder)));
    },
    _postObjToFeed: function(obj, session) {
	  return new SocialKit.DbObj(JSON.parse(Musubi_android_platform._postObjToFeed(JSON.stringify(obj), session)));
    },
    _postObjToSubfeed: function(obj, session, parentId) {
      return JSON.parse(Musubi_android_platform._postObjToSubfeed(JSON.stringify(obj), session, parentId));
    },
    _urlForRaw: function(objId) {
...
SocialKit.Feed.prototype._newMessage = function(json) {
    if (this._messageListener != null) {
        var msg = new SocialKit.Message(json);
...
SocialKit.DbObj.prototype.init = function(json) {
    ...
    this.parentObjId = json.parentObjId;
    this.type = json.type;
    this.json = json.json;
    this.data = this.json
    this.raw_data_url = json.raw_data_url;
    ...
    this.session = json.session;
    this.timestamp = new Date(parseInt(json.timestamp));
};

SocialKit.DbObj.prototype.query = function(query, sortOrder) {
   return Musubi.platform._querySubfeed(this.objId, query, sortOrder);
}

SocialKit.DbObj.prototype.post = function(obj) {
	Musubi.platform._postObjToSubfeed(obj, this.session, this.objId);
};

SocialKit.DbObj.prototype.toString = function() {
    return "<DbObj: " + this.objId + "," + this.timestamp + "," + this.sender.toString() + "," + this.appId + "," + this.session + "," + this.type + "," + ((this.json != null) ? JSON.stringify(this.json) : null) + ">";
};

SocialKit.Message = SocialKit.DbObj;
